...
from app.models.schemas import (
    Holding, TradeHistory, PortfolioPerformance, PortfolioDetailResponse, TradeCreate
)
...
class PortfolioService:
    ...
    def add_trade(self, trade: TradeCreate):
        ...
        try:
            ...
            if current_holding:
                ...
                if trade.side == 'BUY':
                    ...
                    if current_qty <= 0:
                        ...
                        new_avg_cost = new_proceeds / abs(new_quantity) if new_quantity != 0 else 0
                    else:
            ...
            # Execute Insert
            self.db.execute_command(query_insert, (
                display_symbol, trade.symbol_key, trade.market, trade.trade_date, trade.side, 
                trade.shares, trade.price, total_amount, trade.currency, 
                trade.fx_rate_to_jpy, trade.asset_type, trade.memo, realized_pnl
            ))
            
            # Insert/Update Holding
            self._upsert_holding(
                display_symbol, trade.symbol_key, trade.market, trade.asset_type, 
                new_quantity, new_avg_cost, trade.currency
            )
    ...
    def delete_trade(self, trade_id: int):
        ...
        try:
            ...
            self.db.execute_command("DELETE FROM trades WHERE trade_id = %s", (trade_id,))
    ...
    def _upsert_holding(self, symbol, symbol_key, market, asset_type, quantity, avg_cost, currency):
        if quantity == 0:
            ...
            query = """
                ...
                ON CONFLICT (symbol) DO UPDATE SET
                    ...
                    updated_at = NOW()
            """
            self.db.execute_command(query, (symbol, symbol_key, market, asset_type, quantity, avg_cost, currency))
    ...
    def _rebuild_holding(self, symbol, symbol_key=None):
        ...
        self._upsert_holding(symbol, last_symbol_key, market, asset_type, quantity, avg_cost, currency)
    ...
    def _get_holdings(self, fx_rate: Decimal) -> list[Holding]:
        ...
        for r in rows:
            ...
            lookup_key = symbol_key if symbol_key else symbol # Should be symbol_key after migration
            ...
            name = symbol
            if lookup_key:
                name_res = self.db.execute_query("SELECT name FROM instruments WHERE symbol_key = %s", (lookup_key,))
                if name_res:
                    name = name_res[0][0]

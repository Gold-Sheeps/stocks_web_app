...
def main():
    symbol = "NVDA"
    print(f"Recalculating indicators for {symbol}...")
    ...
    try:
        ...
        # 1. Price Data (LIMIT 1000 to ensure enough history)
        query = """
        ...
        # DataFrame setup (Sort ASC for calc)
        ...
        for idx, row in df.iterrows():
            ...
            # Print sample for latest date
            if idx == df.index[-1]:
                print(f"Latest Calc: Date={row['trading_date']}, SMA20={row['sma20']}, RSI={row['rsi14']}, High52={row['high_52w']}, SMA5={row['sma5']}")
            
            insert_query = """
                INSERT INTO indicator_daily (
                    symbol_key, trading_date,
                    sma5, sma20, sma50, sma200, ema21, ema50,
            ...
            params = [
                symbol,
                row['trading_date'],
                None if pd.isna(row['sma5']) else Decimal(str(row['sma5'])),
            ...
            if db.execute_command(insert_query, tuple(params)):
                inserted += 1
        
        print(f"Success! Inserted/Updated {inserted} rows for {symbol}.")
        
        # Verify immediately
        verify = db.execute_query("SELECT sma20, rsi14 FROM indicator_daily WHERE symbol_key=%s ORDER BY trading_date DESC LIMIT 1", (symbol,))
        print(f"Verification Check in DB V3: {verify}")
